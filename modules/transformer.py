...
class EncoderLayer(tf.keras.layers.Layer):
  ...
  def __init__(self, d_model, num_heads, dff, rate=0.1, doMask = False, seed_value = 42):
    ...
    self.mha =  tf.keras.layers.MultiHeadAttention(num_heads=num_heads, key_dim=d_model)#, dropout=0.3) # TODO ATTENTION DROPOUT HERE
  ...
  def call(self, x1, mask=None, adminSum=0, training=None):
    if self.doMask:
...
class Encoder(tf.keras.layers.Layer):
  ...
  def call(self, xa, training=None):
    if self.doMask:
        x, mask = xa
    else:
        x = xa
    ...
    for i in range(self.num_layers):
        if self.doMask:
          xF = self.enc_layers[i](x, mask, self.adminSumer, training=training)
        else: 
          xF = self.enc_layers[i](x, adminSum=self.adminSumer, training=training)
