...
def getWeightName(number, name, fold, 
                learning=False, results=True,
                architecture='transformer', abstractionType=None, symbols=0, 
                layers=None, header=None,  dmodel=None, dff=None, 
                num_resblocks=None, num_channels=None, use_1x1conv=True, 
                dropout=None, strategy=None,  num_coef=None,):
    """ building saving name for model weights """
    if results:
        baseName = "./results/results-" + str(int(number)) + '_' + name + ' -a ' + architecture + ' -m ' + abstractionType
    else:
        baseName = "./saves/weights-" + str(int(number)) + '_' + name + ' -a ' + architecture + ' -m ' + abstractionType + ' -f ' + str(fold)

    if (architecture == 'transformer'):
        baseName += ' -s ' + str(int(symbols)) 
        baseName += ' -l ' + str(int(layers))
        baseName += ' -h ' + str(int(header))
        baseName += ' -dm ' + str(int(dmodel))
        baseName += ' -df ' + str(int(dff))


    if architecture == 'resnet':
        baseName += ' -s ' + str(int(symbols)) 
        baseName += ' -rb ' + str(int(num_resblocks)) 
        baseName += ' -ch ' + str(int(num_channels))
        if use_1x1conv:
    ...
    if dropout is not None:
        baseName = baseName + ' -dr ' + str(dropout)

    if strategy is not None:
        baseName = baseName + ' -st ' + str(strategy)[0]
        
    if num_coef is not None:
        baseName = baseName + ' -cf ' + str(int(num_coef))
...
def doAbstractedTraining(trainD, valD, testD, y_train1, y_val, y_testy, 
                        BATCH, seed_value, num_epochs,
                        num_of_classes, number, dataName, fold, 
                        symbolCount, 
                        numOfAttentionLayers, dmodel, 
                        header, dff, skipDebugSaves=False, patience=0, saveWeights=False, abstractionType=None, rate=0.0, ncoef=0,
    ...
    if architecture == 'transformer':    
        ...
        n_model2.fit(x_trains2, y_train1, validation_data = (x_vals2, y_val) , epochs = num_epochs, batch_size = BATCH, verbose=TF_VERBOSE, callbacks =[earlystop], shuffle = False)
        
        if saveWeights:
            n_model2.save_weights(getWeightName(number, dataName, fold,
                                                learning=True, results=False,
                                                architecture=architecture, abstractionType=abstractionType, symbols=symbolCount,
                                                ...
                                                dropout=rate, strategy=abstractionType, num_coef=ncoef), 
                                                overwrite=True)
        ...
        if saveWeights:
            model.save_weights(getWeightName(number, dataName, fold,
                                                learning=True, results=False,
                                                architecture=architecture, abstractionType=abstractionType, symbols=symbolCount,
                                                layers=numOfAttentionLayers, header=header, dmodel=dmodel, dff=dff,
                                                num_resblocks=num_resblocks, num_channels=num_channels, use_1x1conv=use_1x1conv,
...
def preprocessData(x_train1, x_val, x_test, y_train1, y_val, y_test, y_trainy, y_testy, 
                   binNr, symbolsCount, dataName, seqSize, method='ORI', strategy='uniform', ncoef=None, coef_div=None):    
    """ Preprocess the data """
